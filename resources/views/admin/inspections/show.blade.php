...
@section('content')
@include('admin.inspections.damage-assessment')
...
<div class="table-responsive"><table class="table table-sm align-middle"><thead><tr><th>Item</th><th>Required</th><th>Check-in found / damaged</th><th>Found now</th><th>Damaged</th><th>Missing</th><th>New loss / damage</th><th>Estimate AED</th><th>Notes</th></tr></thead><tbody>
@foreach($inventoryRows as $row)<tr><td>{{ $row['room'] }} — {{ $row['name'] }}</td><td>{{ $row['required'] }}</td><td>{{ $row['baseline_found'] ?? '—' }} / {{ $row['baseline_damaged'] ?? '—' }}</td><td>{{ $row['found'] ?? 'Pending' }}</td><td>{{ $row['damaged'] ?? 'Pending' }}</td><td>{{ $row['missing'] ?? '—' }}</td><td>{{ $row['new_missing'] ?? '—' }} / {{ $row['new_damaged'] ?? '—' }}</td><td>{{ isset($row['estimate']) ? number_format($row['estimate'],2) : 'No baseline' }}</td><td>{{ $row['notes'] ?? '' }}</td></tr>@endforeach
