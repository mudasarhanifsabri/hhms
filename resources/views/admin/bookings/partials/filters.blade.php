<form id="bookingFilters" method="GET" action="{{ url()->current() }}" class="{{ ($embedded ?? false) ? 'border-bottom' : 'card mb-3' }}">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            @endunless
            <div class="col d-flex gap-2 align-items-center flex-wrap"><button class="btn btn-sm btn-primary">Apply Filters</button><a href="{{ url()->current() }}" class="btn btn-sm btn-outline-secondary">Reset</a>
                @unless($embedded ?? false)<span class="small text-muted ms-md-auto">Showing {{ $bookings->firstItem() ?? 0 }}–{{ $bookings->lastItem() ?? 0 }} of {{ $bookings->total() }} matching bookings</span>@endunless
            </div>
        </div>
    </div>
</form>
