...
<div class="card">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="bg-light-subtle"><tr><th>Date</th><th>No.</th><th>Category</th><th>Unit</th><th>Supplier</th><th>Responsibility</th><th>VAT</th><th>Total</th><th>Receipt</th></tr></thead>
            <tbody>
            @forelse($expenses as $expense)
                <tr>
                    ...
                    <td><span class="badge bg-light text-dark">{{ str_replace('_', ' / ', ucfirst($expense->responsibility)) }}</span></td>
                    <td>AED {{ number_format((float) $expense->vat_amount, 2) }}</td>
                    <td>AED {{ number_format((float) $expense->gross_amount, 2) }}</td>
                    <td>@if($expense->receipt_path)<a href="{{ asset('storage/'.$expense->receipt_path) }}" target="_blank" class="btn btn-sm btn-soft-primary"><i class="ri-attachment-2"></i></a>@else - @endif</td>
                </tr>
            @empty
                <tr><td colspan="9" class="text-center text-muted py-4">No expenses recorded.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal fade" id="expenseModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable"><form class="modal-content" method="post" action="{{ route('admin.accounting.expenses.store') }}" enctype="multipart/form-data">@csrf
        ...
        <div class="modal-body row g-3">
            ...
            <div class="col-12"><label class="form-label">Receipt / Invoice</label><input type="file" name="receipt" class="form-control"></div>
        </div>
